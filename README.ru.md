...
- Запуск, пауза и остановка таймера в один клик, с хоткеями (⌘/Ctrl+Enter — начать или сохранить, Space — пауза)
- Классификация задач по тегам
- Ставка по тегу в любой из валют (₽ / $ / €) — каждая запись сохраняет свою валюту, переключение не переписывает историю
...
- Ручное добавление, редактирование и удаление записей задним числом, с предупреждением, если время новой записи пересекается с уже существующей
- **Выставление счёта в PDF** — по тегу (проекту) или сразу по всем записям за период, период выбирается так же, как в Истории, реквизиты свои и клиента вводятся свободно (без привязки к формату конкретной страны), налоговая ставка автоматически увеличивает сумму счёта так, чтобы после уплаты налога на руках осталось ровно то, что заработано, подпись — рисунком или загруженным изображением
- **История счетов** — каждый выставленный счёт сохраняется с проектом, периодом и суммой, можно скачать его повторно, отредактировать или удалить, найти по проекту, а при повторном выставлении по тому же проекту и периоду приложение предупредит, чтобы не выставить счёт дважды случайно
- Русский и английский интерфейс, переключается в любой момент
- Светлая и тёмная тема, адаптивный интерфейс
